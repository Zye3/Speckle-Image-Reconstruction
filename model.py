...
#         # Encoder layers
#         self.encoder = nn.Sequential(
#             nn.Conv2d(1, 32, kernel_size=3, padding=1),   # channel:1->32, size remains 32x32
#             nn.BatchNorm2d(32),
#             nn.ReLU(),
#             nn.MaxPool2d(kernel_size=2, stride=2),        # size: 32x32 -> 16x16
#
#             nn.Conv2d(32, 64, kernel_size=3, padding=1),  # channel:32->64
#             nn.BatchNorm2d(64),
#             nn.ReLU(),
#             nn.MaxPool2d(kernel_size=2, stride=2),        # size: 16x16 -> 8x8
#
#             nn.Conv2d(64, 128, kernel_size=3, padding=1), # channel:64->128
#             nn.BatchNorm2d(128),
#             nn.ReLU(),
#             nn.MaxPool2d(kernel_size=2, stride=2)         # size: 8x8 -> 4x4
#         )
#
#         # Decoder layers
#         self.decoder = nn.Sequential(
#             nn.ConvTranspose2d(128, 64, kernel_size=2, stride=2),  # size: 4x4 -> 8x8
#             nn.BatchNorm2d(64),
#             nn.ReLU(),
#
#             nn.ConvTranspose2d(64, 32, kernel_size=2, stride=2),   # size: 8x8 -> 16x16
#             nn.BatchNorm2d(32),
#             nn.ReLU(),
#
#             nn.ConvTranspose2d(32, 16, kernel_size=2, stride=2),   # size: 16x16 -> 32x32
#             nn.BatchNorm2d(16),
#             nn.ReLU(),
#
#             nn.Conv2d(16, 1, kernel_size=3, padding=1),  # channel:16->1, size remains 32x32
#             nn.Sigmoid()
#         )
#
#     def forward(self, x):
...
'''
Variational Autoencoder # Use 64x64 or 256x256
'''


class Sampling(torch.nn.Module):
    """
    This class implements the reparameterization trick by sampling from a normal distribution and combining it with z_mean and z_log_var.
    这个类实现了重参数技巧，通过从正态分布中采样并将其与 z_mean 和 z_log_var 结合。
    """

    def forward(self, inputs):
        """
        Performs the forward pass of the sampling layer.
        执行采样层的前向传递。
        """
...
class Encoder(nn.Module):
    def __init__(self, latent_dims=2):
        ...
        # Adjusting convolutional layers to handle 256x256 input
        self.conv_layers = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1),  # Output: 32x64x64, 32x256x256,
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),  # Output: 32x32x32, 32x128x128, 32x64x64
            nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),  # Output: 64x32x32, 64x128x128,
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),  # Output: 64x16x16, 64x64x64,
            nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),  # Output: 128x16x16, 128x64x64
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),  # Output: 128x8x8, 128x32x32
            nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1),  # Output: 256x8x8, 256x32x32
            nn.BatchNorm2d(256),
            # nn.ReLU(),
            # nn.MaxPool2d(2, 2),  # Output: 256x4x4, 256x16x16
            # nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1),  # Output: 512x4x4, 512x16x16
            # nn.BatchNorm2d(512),
            # nn.ReLU(),
            # nn.MaxPool2d(2, 2),  # Output: 512x4x4
            # nn.Conv2d(512, 1024, kernel_size=3, stride=1, padding=1),  # Output: 1024x4x4
            # nn.BatchNorm2d(1024),
            nn.ReLU(),
            nn.Flatten()
        )
        self.fc_mean = nn.Linear(256 * 8 * 8, latent_dims)  # 1024x4x4
        self.fc_log_var = nn.Linear(256 * 8 * 8, latent_dims)  # 1024x4x4
...
class Decoder(nn.Module):
    def __init__(self, latent_dims=4096):
        ...
        self.fc = nn.Linear(latent_dims, 256 * 8 * 8)  # 1024x4x4 if the input is 64x64 ot 256x256

        self.conv_layers = nn.Sequential(
            # nn.ConvTranspose2d(1024, 512, kernel_size=3, stride=2, padding=1, output_padding=1),  # Output:512x8x8
            # nn.BatchNorm2d(512),
            # nn.ReLU(),
            # nn.ConvTranspose2d(512, 256, kernel_size=3, stride=2, padding=1, output_padding=1),  # Output: 256x8x8,
            # nn.BatchNorm2d(256),
            # nn.ReLU(),
            nn.ConvTranspose2d(256, 128, kernel_size=3, stride=1, padding=1, output_padding=1),  # Output: 128x16x16
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.ConvTranspose2d(128, 64, kernel_size=3, stride=1, padding=1, output_padding=1),  # Output: 64x32x32
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.ConvTranspose2d(64, 32, kernel_size=3, stride=1, padding=1, output_padding=1),  # Output: 32x64x64
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.Conv2d(32, 1, kernel_size=3, stride=1, padding=1),  # Output: 1x64x64
            nn.Sigmoid()
        )

    def forward(self, x):
        x = self.fc(x)
        x = x.view(-1, 256, 8, 8)  # Reshape to match the corresponding encoder output
        x = self.conv_layers(x)
        return x
...
class VAE(nn.Module):
    """
    Variational Autoencoder (VAE) class that combines the Encoder and Decoder for processing and reconstructing 256x256 images.
    """
